...
COMPANIES.forEach((c) => {
  if (c.keySlot) CLAIMED_KEYS[c.keySlot.toUpperCase()] = { iconUrl: c.iconUrl, name: c.name, url: c.url };
});

// Renders either a favicon or falls back to the letter
function ClaimedKeyIcon({ letter, fallback }: { letter: string; fallback: React.ReactNode }) {
  const claimed = CLAIMED_KEYS[letter.toUpperCase()];
  const [err, setErr] = React.useState(false);
  if (!claimed || err) return <>{fallback}</>;
  return (
    <img
      src={claimed.iconUrl}
      alt={claimed.name}
      title={claimed.name}
      onError={() => setErr(true)}
      className="h-full w-full object-contain p-[1.5px] rounded-sm"
      ...
      <Row>
        ...
        {["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P"].map((letter) => (
          <Key key={letter} keyCode={`Key${letter}`}>
            <ClaimedKeyIcon letter={letter} fallback={letter} />
          </Key>
        ))}
