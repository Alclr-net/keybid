...
function BidFormContent({ keySlot }: { keySlot: string }) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#080a10] text-white flex flex-col items-center select-none">
      {/* ── Focused Top Navigation ── */}
      <header className="w-full border-b border-white/10 bg-[#0c0f18]/80 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link
            href="/"
            className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-zinc-400 hover:text-white transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            <span className="font-extrabold text-sm sm:text-base tracking-tight text-white">
              KEY<span className="text-blue-500">BID</span>
            </span>
          ...
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[11px] font-semibold">
            <IconLock size={12} />
      ...
      <main className="w-full max-w-xl mx-auto px-4 py-8 sm:py-12 flex-1 flex flex-col justify-center">
        {paymentSuccess ? (
          ...
          <div className="rounded-3xl bg-[#0e121d] border border-emerald-500/30 p-8 sm:p-10 text-center shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="w-16 h-16 rounded-full bg-emerald-500/15 border-2 border-emerald-500/40 text-emerald-400 flex items-center justify-center mx-auto text-3xl shadow-[0_0_24px_rgba(16,185,129,0.3)]">
              <IconCheck size={36} stroke={3} />
            </div>

            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-bold font-mono uppercase tracking-wider mb-2">
              ...
              <p className="text-xs sm:text-sm text-zinc-400 mt-2 max-w-md mx-auto leading-relaxed">
                Congratulations, <strong className="text-white">{paymentSuccess.brandName}</strong>. Your bid has been recorded on the live auction ledger.
              </p>
            </div>

            {/* Receipt Summary Box */}
            <div className="rounded-2xl bg-black/40 border border-white/10 p-4 text-xs font-mono space-y-2 text-left">
              <div className="flex justify-between text-zinc-400">
                <span>Hardware Slot:</span>
                <span className="text-white font-bold">Keycap [{paymentSuccess.keySlot}]</span>
              </div>
              <div className="flex justify-between text-zinc-400">
                <span>Amount Paid:</span>
                <span className="text-emerald-400 font-bold">${paymentSuccess.bidAmount} USD</span>
              </div>
              <div className="flex justify-between text-zinc-400">
                <span>Payment Reference:</span>
                <span className="text-zinc-300 truncate max-w-[200px]">{paymentSuccess.paymentId}</span>
              </div>
              <div className="flex justify-between text-zinc-400">
                <span>Order ID:</span>
                <span className="text-zinc-300 truncate max-w-[200px]">{paymentSuccess.orderId}</span>
              </div>
            </div>

            {/* Guarantee Note */}
            <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-300 text-xs text-left flex items-start gap-2">
              <IconShieldCheck size={18} className="shrink-0 text-blue-400 mt-0.5" />
              <span>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
              ...
              <Link
                href="/auction"
                className="w-full sm:flex-1 py-3 px-4 rounded-xl font-bold text-xs sm:text-sm bg-zinc-800 hover:bg-zinc-700 text-white border border-white/10 transition-all text-center cursor-pointer"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl bg-[#0e121d] border border-white/10 p-5 sm:p-6 shadow-xl relative overflow-hidden">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3 sm:gap-4">
                  {/* 3D Keycap Icon */}
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-b from-zinc-800 to-zinc-950 border-2 border-blue-500/80 shadow-[0_8px_20px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.2)] flex flex-col items-center justify-center p-1 shrink-0 relative">
                    <span className="font-mono text-xl sm:text-2xl font-black text-white">
                      {keySlot}
                    </span>
                    <span className="text-[8px] font-mono text-zinc-400 font-bold uppercase">
                      KEYCAP
                    </span>
                  </div>
                ...
                <div className="text-right shrink-0 bg-black/40 border border-white/5 p-2.5 rounded-2xl">
                  <div className="text-[10px] font-mono uppercase text-zinc-500">
                    Current Highest
                  </div>
                  <div className="font-mono text-lg sm:text-xl font-extrabold text-blue-400">
                    {loadingLive ? (
                      <span className="animate-pulse">Loading…</span>
                    ) : currentHighest > 0 ? (
                      `$${currentHighest}`
                    ) : (
                      <span className="text-zinc-400">$0 (Open)</span>
                    )}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    {liveData.currentHolder.iconUrl && (
                      ...
                      <img
                        ...
                        className="w-5 h-5 rounded object-contain bg-black/60 p-0.5"
                        onError={(e) => {
                    ...
                    <span className="text-zinc-300 truncate">
                      Held by <strong className="text-white">{liveData.currentHolder.name}</strong>
                    </span>
            ...
            {outbidAlert && (
              <div className="p-4 rounded-2xl bg-amber-500/15 border-2 border-amber-500/40 text-amber-200 text-xs flex items-start gap-3 shadow-lg animate-in shake">
                <IconAlertCircle size={20} className="text-amber-400 shrink-0 mt-0.5" />
                <div className="flex-1 space-y-1">
                  <div className="font-bold text-amber-300">Live Outbid Detected</div>
                  <p>{outbidAlert.message}</p>
                </div>
                <button
                  type="button"
            ...
            <form onSubmit={handleInitiatePayment} className="space-y-5">
              ...
              <div className="rounded-2xl bg-[#0e121d] border border-white/10 p-5 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-mono text-zinc-500">
                    Minimum next bid: <strong className="text-blue-400">${minimumAllowedBid}</strong>
                  </span>
                </div>

                <div className="relative flex items-center rounded-2xl border-2 border-blue-500 bg-black/60 px-4 py-3 shadow-inner">
                  <span className="text-xl sm:text-2xl font-mono font-bold text-blue-400 select-none pr-2">
                    $
                  </span>
                  <input
                    type="number"
                    ...
                    required
                    className="w-full bg-transparent font-mono text-2xl sm:text-3xl font-black text-white outline-none"
                  />
                  <span className="text-xs font-mono uppercase text-zinc-500">USD</span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-1 flex-wrap">
                  <span className="text-[10px] font-mono uppercase text-zinc-500">Quick Bid:</span>
              ...
              <div className="rounded-2xl bg-[#0e121d] border border-white/10 p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-white/10 bg-black/40 px-3.5 py-2.5 text-xs sm:text-sm text-white placeholder:text-zinc-600 outline-none focus:border-blue-500 transition-colors"
                    />
                  </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-zinc-300 mb-1">
                    Website URL <span className="text-zinc-500 font-normal">(Optional · for live backlink)</span>
                  </label>
                  <input
                    type="url"
                ...
                <div>
                  <label className="block text-xs font-semibold text-zinc-300 mb-1">
                    Decal Logo <span className="text-zinc-500 font-normal">(Optional · PNG, SVG)</span>
                  </label>
                  <label className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-white/15 hover:border-blue-500 bg-black/30 cursor-pointer transition-colors">
                    {logoPreview ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={logoPreview}
              ...
              <div className="rounded-2xl bg-[#0e121d] border border-white/10 p-5 space-y-3">
                ...
                <div className="space-y-2 text-xs font-mono">
                  <div className="flex justify-between text-zinc-400">
                    ...
                    <span className="text-white font-bold">${bidAmount}</span>
                  </div>
                  <div className="flex justify-between text-zinc-400">
                  ...
                  <div className="pt-2 border-t border-white/10 flex justify-between text-sm sm:text-base font-bold text-white">
                    ...
                    <span className="text-blue-400 font-black">${bidAmount} USD</span>
                  </div>
                </div>
              ...
              <div className="p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 space-y-1">
                <div className="font-bold flex items-center gap-1.5 text-blue-200">
                  <IconRotateCcw size={14} className="text-blue-400" />
                  100% Outbid Refund Guarantee
                </div>
                <p className="text-[11px] leading-relaxed text-blue-300/90 font-normal">
              ...
              <div className="p-3.5 rounded-2xl bg-black/40 border border-white/10">
                <label className="flex items-start gap-3 cursor-pointer text-xs text-zinc-300 leading-normal">
                  <input
                    type="checkbox"
                    checked={termsAgreed}
                    onChange={(e) => setTermsAgreed(e.target.checked)}
                    required
                    className="mt-0.5 h-4 w-4 rounded border-zinc-700 bg-zinc-900 text-blue-600 focus:ring-blue-500 cursor-pointer"
                  />
                  <span>
              ...
              <button
                ...
                className={cn(
                  ...
                  canSubmit
                    ? "bg-blue-600 hover:bg-blue-500 active:bg-blue-700 shadow-[0_4px_24px_rgba(37,99,235,0.4)] active:scale-[0.99]"
