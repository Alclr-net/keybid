...
function HeroSection() {
    ...
    return (
        ...
        <section className="w-full pt-8 sm:pt-16 md:pt-22 pb-8 sm:pb-14 px-4 sm:px-6 flex flex-col items-center text-center">
            {/* Live Pool Pill */}
            <ScrollReveal delay={0.05} distance={15}>
                <button className={cn("group inline-flex items-center gap-2 px-1 py-1 mb-4 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-white/10 transition-all text-xs sm:text-sm font-medium text-zinc-700 dark:text-zinc-300 shadow-2xs hover:border-zinc-300 dark:hover:border-white/20")}>
                    <span className="flex items-center gap-2">
                        ...
                        <span className="pr-1 sm:pr-2 text-zinc-700 dark:text-zinc-300 text-xs sm:text-sm">Bidding just went live!</span>
                    </span>
                </button>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.1} blurAmount={12}>
                <h1 className="font-display text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight text-zinc-950 dark:text-white leading-[1.14] sm:leading-[1.08] max-w-4xl text-shadow-sm uppercase">
                    Claim{" "}
                    <CanvasText
                        text="the key"
            ...
            <ScrollReveal delay={0.18} blurAmount={8}>
                <p className="mt-3 sm:mt-4 text-xs sm:text-base md:text-lg text-zinc-600 dark:text-zinc-400 font-normal max-w-2xl leading-relaxed px-2">
            ...
            <ScrollReveal delay={0.25} distance={20}>
                <div className="mt-6 sm:mt-10 w-full max-w-2xl mx-auto">
                    ...
                    <div className="mt-4 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-[10px] sm:text-xs text-zinc-500 dark:text-zinc-400 font-mono select-none px-2">
                        <Ping>
                            <span className="whitespace-nowrap">{keysList.length > 0 ? `${keysList.length} keys claimed` : "Live keys registry"}</span>
                        </Ping>
                        <Ping color={"bg-blue-600"}>
                            <span className="text-zinc-700 dark:text-zinc-300 font-semibold whitespace-nowrap">
                                ${keysList.reduce((acc, k) => acc + (k.current_bid_amount || 0), 0)} in active bids
                            </span>
                        </Ping>
                        <Ping color={"bg-red-500"}>
                            <span className="whitespace-nowrap">Daily live desk placement</span>
                        </Ping>
                    </div>
                </div>
            </ScrollReveal>
